...
- **AWS 预期回应：** 建议使用 Amazon Cognito 或外部 IdP（如 Okta/Keycloak）集成，配合 **AWS STS `AssumeRoleWithWebIdentity`** 兑换临时凭证，并在 IAM Policy 中通过 `${aws:PrincipalTag/...}` 挂载 ABAC 标签。
    
- ****STS 临时凭证加 ABAC 是标准做法，但工程落地时有两个硬伤：**
...
- **AWS 预期回应：** 推崇在 Bedrock Native 侧直接配置 Bedrock Guardrails，通过其 PII 识别、Prompt Attack 检测和敏感词过滤；推荐开启 Stream 模式降低感知延迟。
- **Bedrock 原生 Guardrails 做首尾两端的检测很成熟，但对 MCP Tool 场景略有欠缺：**
    > 
...
- **AWS 预期回应：** 主推 **AWS Step Functions (Distributed Map)** 编排，或者利用 Bedrock Agents 的内置 Session State，配合 DynamoDB 记录对话上下文。
- **Step Functions 适合确定性的 Dag 流程，但用它强行编排自主 Agent 会陷入 ‘状态机爆炸：**
    > 
...
- **AWS 预期回应：** 推荐结合 Amazon EventBridge 捕获高危事件，触发 SNS/Lambda 向管理员发送确认通知，或使用 AppSync / Step Functions Task Token 实现异步挂起与回调（Callback）。
- **Task Token 挂起机制思路很对，但在高频/高敏业务场景（如电商改价、代码库 Merge）下，单纯的二次确认容易引发 ‘审批疲劳’。** **我们更关注：** **授权凭证化（Mandate-based HITL）与防篡改审计：**
    > 
...
- **AWS 预期回应：** 推荐使用 **AWS Cloud Map** 进行服务发现，配合 **VPC Lattice** 提供跨 VPC 的 Layer 7 路由、安全隔离与流量控制。
